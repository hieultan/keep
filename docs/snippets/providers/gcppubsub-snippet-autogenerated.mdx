...
- **service_account_json**: Service account JSON with permissions to read the subscription (required: False, sensitive: True)
- **subscription**: Pub/Sub subscription path (required: True, sensitive: False)

Certain scopes may be required to perform specific actions or queries via the provider. Below is a summary of relevant scopes and their use cases:
- **pubsub.subscriber**: Read access to the Pub/Sub subscription (mandatory)
